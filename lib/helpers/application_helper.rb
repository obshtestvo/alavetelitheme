...
module ApplicationHelper
    ...
    def event_description(event)
        ...
        when 'sent'
            _('Request sent to {{public_body_name}} by {{info_request_user}} &nbsp; <span class="icon calendar"></span> {{date}}.',
        ...
        when 'followup_sent'
            ...
            when 'internal_review'
                _('Internal review request sent to {{public_body_name}} by {{info_request_user}} &nbsp; <span class="icon calendar"></span> {{date}}.',
                 :public_body_name => body_link,
                 :info_request_user => user_link,
                 :date => date)
            when 'waiting_response'
                _('Clarification sent to {{public_body_name}} by {{info_request_user}} &nbsp; <span class="icon calendar"></span> {{date}}.',
                 :public_body_name => body_link,
                 :info_request_user => user_link,
                 :date => date)
            else
                _('Follow up sent to {{public_body_name}} by {{info_request_user}} &nbsp; <span class="icon calendar"></span> {{date}}.',
                 :public_body_name => body_link,
                 :info_request_user => user_link,
                 :date => date)
            end
        when 'response'
            _('Response by {{public_body_name}} to {{info_request_user}} on {{date}}.',
             ...
             :date => date)
        when 'comment'
            _('Request to {{public_body_name}} by {{info_request_user}}. Annotated by {{event_comment_user}} &nbsp; <span class="icon calendar"></span> {{date}}.',
             :public_body_name => body_link,
             :info_request_user => user_link,
